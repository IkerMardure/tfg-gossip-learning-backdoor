...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate a publication-ready Mean Attack Success Rate (ASR) comparison plot.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--fc", required=True, type=Path, help="Raw log file for the Fully Connected topology")
    parser.add_argument("--gateway", required=True, type=Path, help="Raw log file for the SBM Gateway topology")
    parser.add_argument("--peripheral", required=True, type=Path, help="Raw log file for the SBM Peripheral topology")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("mean_asr_comparison.png"),
...
def main() -> None:
    ...
    gateway_series = load_mean_asr(args.gateway)
    peripheral_series = load_mean_asr(args.peripheral)

    if not any((fc_series, gateway_series, peripheral_series)):
