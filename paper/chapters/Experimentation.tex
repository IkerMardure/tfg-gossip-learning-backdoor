...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/SBM_8_graph.png}
...
The primary experiment was conducted using the MNIST dataset \cite{lecun1998gradient}. To align with the architectural requirements of the model, all images were resized to $32 \times 32$ pixels. The classification task was constrained to a subset of 5 classes, primarily to lower computational costs. To establish a controlled and uniform baseline, the dataset was partitioned in an Independent and Identically Distributed (IID) manner among the clients. This ensures that each node starts with a statistically similar subset of the data.

A modified LeNet \cite{lecun1998gradient} architecture was used as the local model across all nodes. The network is composed of three convolutional layers of 16, 32, and 64 filters, respectively, each with $3 \times 3$ kernels and a padding of 1. Each convolutional operation is followed by a ReLU activation function and a $2 \times 2$ Max Pooling operation. The resulting feature maps are processed through two fully connected layers, the first containing 128 units and the last one containing as many units as the classes used (in this case, 5). Furthermore, a dropout layer with a 0.2 probability was inserted prior to the final classification layer to mitigate overfitting during the training.
...
While the theoretical framework of Gossip Learning often assumes parallel execution across all participating nodes (as outlined in Algorithm 2.2), simulating such a decentralized network on centralized hardware requires specific implementation constraints. 

In our experimental setup using the customed Flower framework \cite{belenguer2025glow}, the execution flow was implemented as an asynchronous, sequential process. Specifically, a single communication round is strictly defined as the local training and subsequent parameter broadcast of exactly one client. During its designated round, the active client performs local optimization and immediately sends its updated model to its topological neighbors, who aggregate it into their own local state.

To ensure a fair and balanced evaluation, it was imperative that every node in the network received an equal amount of training exposure. Therefore, the total number of communication rounds was scaled linearly based on the network size to achieve 20 full network training cycles. Consequently:
\begin{itemize}
    \item For the 5-node topologies (Fully Connected, Ring, Star), the simulation spanned exactly 100 communication rounds ($5 \text{ nodes} \times 20 \text{ cycles}$).
    \item For the scaled 8-node Stochastic Block Model (SBM), the simulation was proportionally extended to 160 communication rounds ($8 \text{ nodes} \times 20 \text{ cycles}$).
\end{itemize}

This sequential execution accurately models the asynchronous nature of real-world peer-to-peer networks, where nodes do not wait for a global synchronization barrier, but rather update and broadcast their parameters independently.